...
//The first part of the if statement starts the timer and will open a card when clicked. It will push the opened card to the openCards array to check if there is a match once a second card is clicked
  if (openCards.length === 0) {
      $(this).toggleClass('open show');
      openCards.push($(this));
      timeStart = true;
    }
//The second part of this if statement will only happen if there is already one card open. It also will not occur if you click the same card twice. Once a second card is opened it will compare whether or not the two cards match, it will increase the score, and will check if all of the cards are matched.
...
function winPopUp() {
  //This triggers the modal when you win. If you click play again it resets the game. If you click the "x" button it will close the modal.
